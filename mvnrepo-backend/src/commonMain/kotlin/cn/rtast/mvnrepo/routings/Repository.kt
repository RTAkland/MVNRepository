...
fun storagePackage(structure: PackageStructure) {
    val versionPath =
        "$STORAGE_PATH/${structure.repository}/${structure.artifactGroup}/${structure.artifactId}/${structure.artifactVersion}/${structure.artifactName}"
...
//    val versionPath = File(artifactPath, structure.artifactVersion).apply { mkdirs() }
    File(versionPath).apply { writeBytes(structure.artifactByteArray!!) }
    val mavenMetadataFile =
        File("$STORAGE_PATH/${structure.repository}/${structure.artifactGroup}/${structure.artifactId}/maven-metadata.xml")
    if (!mavenMetadataFile.exists()) {
        val defaultMetadata = MavenMetadata(
            ...
            MavenMetadata.Versioning(
                ...
                System.now().epochSeconds.toFormatedDate()
            )
        )
        mavenMetadataFile.writeBytes(defaultMetadata.toXMLString())
    }
}